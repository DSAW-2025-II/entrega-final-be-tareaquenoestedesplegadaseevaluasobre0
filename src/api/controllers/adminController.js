...
async function suspendUser(req, res, next) {
  try {
    ...
    if (adminId && adminId.toString() === userId.toString()) {
      return res.status(403).json({ code: 'forbidden', message: 'Cannot suspend or reactivate your own account', correlationId: req.correlationId });
    }
    ...
    await res.audit({ actor: { type: 'admin', id: adminId }, action: 'suspend_attempt', entity: { type: 'User', id: userId }, reason, delta: { before: { suspended: true }, after: { suspended: true }, userIdentifier: user.universityId || user.corporateEmail }, correlationId: req.correlationId });
  ...
  await res.audit({ actor: { type: 'admin', id: adminId }, action: 'suspend', entity: { type: 'User', id: userId }, reason, delta: { before, after, userIdentifier: user.universityId || user.corporateEmail }, correlationId: req.correlationId });
    ...
    if (!user.suspended) {
      // idempotent no-op
  await res.audit({ actor: { type: 'admin', id: adminId }, action: 'unsuspend_attempt', entity: { type: 'User', id: userId }, reason, delta: { before: { suspended: false }, after: { suspended: false }, userIdentifier: user.universityId || user.corporateEmail }, correlationId: req.correlationId });
  ...
  await res.audit({ actor: { type: 'admin', id: adminId }, action: 'unsuspend', entity: { type: 'User', id: userId }, reason, delta: { before: beforeU, after: afterU, userIdentifier: user.universityId || user.corporateEmail }, correlationId: req.correlationId });
...
async function publishBan(req, res, next) {
  ...
  await res.audit({ actor: { type: 'admin', id: adminId }, action: 'publish_ban', entity: { type: 'User', id: driverId }, reason, delta: { before, after, userIdentifier: user.universityId || user.corporateEmail }, correlationId: req.correlationId });
...
async function listAudit(req, res, next) {
  try {
    ...
    // But preserve userIdentifier field for audit log display
    const { redactPII } = require('../middlewares/structuredLogger');

    const items = docs.map(d => {
      const delta = d.delta || d.what || null;
      let redactedDelta = null;
      
      // Preserve userIdentifier before redaction
      let userIdentifier = null;
      if (delta) {
        if (delta.userIdentifier) {
          userIdentifier = delta.userIdentifier;
        } else if (delta.after && delta.after.userIdentifier) {
          userIdentifier = delta.after.userIdentifier;
        }
      }
      
      // Redact PII from delta
      redactedDelta = redactPII(delta);
      
      // Restore userIdentifier after redaction
      if (userIdentifier && redactedDelta) {
        if (!redactedDelta.userIdentifier) {
          redactedDelta.userIdentifier = userIdentifier;
        }
        if (redactedDelta.after && !redactedDelta.after.userIdentifier) {
          redactedDelta.after.userIdentifier = userIdentifier;
        }
      }
...
/**
 * Admin: List user reports with filters and pagination
 * GET /admin/reports
 */
async function listReports(req, res, next) {
  try {
    ...
    const [total, docs] = await Promise.all([
      UserReportModel.countDocuments(query),
      UserReportModel.find(query)
        .populate('reportedUserId', 'firstName lastName corporateEmail role')
    ...
    const items = docs
      .filter(report => report.reportedUserId && report.reporterId && report.tripId)
      .map(report => ({
        id: report._id.toString(),
        reportedUser: {
          id: report.reportedUserId._id.toString(),
          firstName: report.reportedUserId.firstName || 'Usuario',
          lastName: report.reportedUserId.lastName || 'Eliminado',
          corporateEmail: report.reportedUserId.corporateEmail || 'N/A',
          role: report.reportedUserId.role || 'passenger'
        },
        reporter: {
...
async function updateReportStatus(req, res, next) {
  try {
    ...
    await res.audit({
      ...
      entity: { type: 'UserReport', id: reportId },
      reason: reason || `Status changed from ${oldStatus} to ${status}`,
      delta: { before: { status: oldStatus }, after: { status } },
      correlationId: req.correlationId
    });
 ...
 * POST /admin/reports/:reportId/send-message
 */
async function sendMessageToReportedUser(req, res, next) {
  try {
    const { reportId } = req.params;
    ...
    const report = await UserReportModel.findById(reportId)
      .populate('reportedUserId', 'firstName lastName corporateEmail universityId')
      .lean();
    ...
    const reportedUserIdentifier = report.reportedUserId.universityId || report.reportedUserId.corporateEmail || reportedUserId;
    ...
    const notification = await NotificationService.createNotification(
      ...
      message,
      {
        reportId: reportId,
        category: report.category,
        tripId: report.tripId?.toString() || null
      },
      req.correlationId
    );
    ...
    await res.audit({
      actor: { type: 'admin', id: adminId },
      action: 'send_message_to_reported_user',
      entity: { type: 'UserReport', id: reportId },
      reason: `Message sent to reported user: ${title}`,
      delta: { 
        reportedUserId,
        userIdentifier: reportedUserIdentifier,
        title,
