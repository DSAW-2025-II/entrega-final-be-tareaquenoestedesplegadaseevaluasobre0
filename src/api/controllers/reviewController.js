...
class ReviewController {
  ...
  async listReviewsForDriver(req, res, next) {
    try {
      ...
      console.log(`[ReviewController] listReviewsForDriver | driverId: ${driverId} | correlationId: ${req.correlationId}`);

      // Verify driver exists and get basic info
      const UserModel = require('../../infrastructure/database/models/UserModel');
      const VehicleModel = require('../../infrastructure/database/models/VehicleModel');
      
      // Convert driverId to ObjectId if needed
      let driverIdObj = driverId;
      if (typeof driverId === 'string' && mongoose.Types.ObjectId.isValid(driverId)) {
        driverIdObj = new mongoose.Types.ObjectId(driverId);
      }
      
      const driver = await UserModel.findById(driverIdObj).lean();
      if (!driver) {
        console.log(`[ReviewController] Driver not found | driverId: ${driverId} | correlationId: ${req.correlationId}`);
        return res.status(404).json({ code: 'not_found', message: 'Driver not found', correlationId: req.correlationId });
      }

      const query = { driverId: driverIdObj, status: 'visible' };

      const pageNum = parseInt(page, 10) || 1;
      const size = Math.min(parseInt(pageSize, 10) || 10, 50);
      const skip = (pageNum - 1) * size;

      console.log(`[ReviewController] Fetching reviews | query:`, query, `| page: ${pageNum} | size: ${size}`);

      const [itemsRaw, total, vehicle] = await Promise.all([
        ...
        VehicleModel.findOne({ driverId: driverIdObj }).lean().catch(err => {
          console.error(`[ReviewController] Error fetching vehicle:`, err);
          return null; // Return null if vehicle fetch fails
        })
      ]);

      console.log(`[ReviewController] Found ${itemsRaw.length} reviews | total: ${total} | vehicle: ${vehicle ? 'found' : 'not found'}`);
      ...
      // Include driver basic info in response
      const driverInfo = {
        id: driver._id.toString(),
        firstName: driver.firstName,
        lastName: driver.lastName,
        profilePhotoUrl: driver.profilePhoto || null
      };
      ...
      if (vehicle) {
        vehicleInfo = {
          ...
          vehiclePhotoUrl: vehicle.vehiclePhotoUrl || null
        };
      }

      console.log(`[ReviewController] Returning response | driver: ${driverInfo.firstName} ${driverInfo.lastName} | vehicle: ${vehicleInfo ? 'yes' : 'no'}`);
      ...
      console.error(`[ReviewController] Error in listReviewsForDriver:`, err);
      console.error(`[ReviewController] Error stack:`, err.stack);
      next(err);
    }
  }
