 ...
 * Public endpoint (no auth required) to get basic user information
 * IMPORTANT: This route must be defined AFTER all /me routes to avoid conflicts
 * IMPORTANT: The route pattern /:userId/public must match exactly - userId must be a valid ObjectId
 */
router.get(
  '/:userId/public',
  generalRateLimiter,
  (req, res, next) => {
    console.log(`[userRoutes] /:userId/public route matched | userId: ${req.params.userId} | path: ${req.path} | originalUrl: ${req.originalUrl}`);
    next();
  },
